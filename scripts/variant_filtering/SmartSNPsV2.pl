...
die "Usage: perl SmartSNPs.pl <VCF> <alignfile> <AltRefRatio> <ReadDepth> <outfile> [<cutoff>]\n" if @ARGV < 5;
...
$alignHashRef = READ_ALIGNSTRINGS($alignFile);


## create output filenames

($outfile = $VCF) =~ s/\.vcf/_SSfilter\.vcf/;

($log = $VCF) =~ s/\.vcf/_failed\.log/; 
...
# Read alignment strings to determine copy nuber of each site in genome
...
sub REPEATS {
  ...
  if(substr($alignHash{$IDnumeric}, $pos-1, 1) > $cutoff) {
...
# remove and count SNPs that are heterozygous regions
